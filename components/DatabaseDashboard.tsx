                      ...
                      <div className="mt-4 space-y-4 border-t border-white/10 pt-4">
                        ...
                        <div className="flex items-center space-x-2 text-xs">
                          ...
                          {session.analysisResults.analysis?.customerSentiment && (
                            <span className={`px-2 py-1 rounded-full text-xs ${getSentimentColor(session.analysisResults.analysis.customerSentiment.sentiment)}`}>
                              Customer: {session.analysisResults.analysis.customerSentiment.sentiment}
                            </span>
                          )}
                          {session.analysisResults.analysis?.agentSentiment && (
                            <span className={`px-2 py-1 rounded-full text-xs ${getSentimentColor(getOverallAgentSentiment(session.analysisResults.analysis.agentSentiment))}`}>
Agent: {(() => {
                                try {
