...
import { safeProp, guardApiResponse } from '../utils/productionSafety';
          ...
          Object.entries(groupedSessions).map(([dateGroup, sessions]) => (
            <div key={dateGroup} className="space-y-3">
              ...
              {sessions.map((session) => (
                <div
                  key={safeProp(session, 'id', `session-${Date.now()}`)}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <h4 className="font-medium text-text-light group-hover:text-primary transition-colors">
                        {safeProp(session, 'sessionName', 'Unnamed Session')}
                      </h4>
                      <div className="flex items-center space-x-4 mt-1 text-sm text-text-light/70">
                        <span className="flex items-center space-x-1">
                          ...
                          <span>{safeProp(session, 'sourceLanguage', 'unknown').toUpperCase()}</span>
                        </span>
                        <span className="flex items-center space-x-1">
                          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                          </svg>
                          <span>{formatDate(safeProp(session, 'createdAt', new Date().toISOString()))}</span>
                        </span>
                        {safeProp(session, 'audioFileName', null) && (
                          <span className="flex items-center space-x-1">
                            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" />
                            </svg>
                            <span className="truncate max-w-32">{safeProp(session, 'audioFileName', '')}</span>
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={(e) => {
                          const sessionId = safeProp(session, 'id', null);
                          if (sessionId) handleDeleteSession(sessionId, e);
                        }}
